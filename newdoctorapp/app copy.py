...
from backend import agent_response, color_code_cells, load_patient_data, parse_lab_pdf
...
def run():
    ...
    with st.sidebar:
        ...
        st.selectbox(
            "Sex",
            ["Male", "Female"],
            index=0 if selected_patient["sex"] == "Male" else 1,
        )
    ...
    with tab3:
        ...
        care_plan = generate_care_plan(selected_patient)
        st.json(care_plan)
        st.markdown("### Care Plan Summary")
        st.dataframe(care_plan)
